...
class Store:
    ...
    def addAProduct(self)-> None:
        ...
        val = input("Är produkten kamera, dator, mic, eller annat? ")

        match val:
            case "kamera":
                self.inventory.addAKamera()
            case "dator":
                self.inventory.addADator()
            case "mic":
                self.inventory.addAMic()
            case _:
                print("DU HAR VALT EN VANLIG PRODUKT")
